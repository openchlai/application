...
function ch_status (ch)
{
	...
	for (var i=10; i<19; i++) 
	{
		if (!ch[i] || ch[i].length<1) continue; 
		st = i;
...
function chan_status (tp, ch) 
{
	...
	var ch2 = null;
	if (ch[AMI.CHAN_UNIQUEID_2] && ch[AMI.CHAN_UNIQUEID_2].length>0) ch2 = re["channels"][ch[AMI.CHAN_UNIQUEID_2]];
...
function chans_pop (ts, pv, pva)
{
	...
	for (var i=0; i<k.length; i++)  // remove closed, hangup channels
	{
		...
		if (chan_a[id].agtk)
		{
			...
			let o = chan_a[id];
			let ch = [];
			ch[AMI.CHAN_UNIQUEID] = id;
			ch[AMI.CHAN_CID_NUM_2] = o["src_address"];
			ch[AMI.CHAN_STATE_HANGUP] = ts;
...
function chans (o, k, ts, pcoll, pv, pva)
{
