var ACT_COUNT = 0;

te["activity_new"] = { div:["w30 ma bd sh__ gw xx y","vddvw"], ev:["_undd"], c:
[
	{ div:[], c:
	[
		{ s:["c x yy n","New Activity"] },
		{ div:["d x"], ac:["","","_uvp","x y h cb","&Cross;"] },
		{ div:["e"] }
	]},
	{ div:[], c:
	[
		{ s:["x t cd","Enter Phone Number"] },
		{ div:["x y"], c:[ { input:["w29 ba x y","","src_address","","text",""] } ] }
	]},
	{ div:[], c:
	[
		{ div:["c x y"], c:[ { ac:["ao btn","chan_add_ld-chan-4","_activity_call","x y w13 gws_ cb tc","Call"] }, { s:["y bd b savl","..."] } ] },
		{ div:["c x y"], c:[ { ac:["ao btn","chan_add_ld-chan-5","_activity_msg","x y w13 gws_ cb tc","Text"] }, { s:["y bd b savl","..."] } ] },
		{ p:["e","o"] }
	]}
]};

// ---------------------------------------------------------------------

te["activity_message_sended"] = { ufn:["activity_message_sended"] };

te["activity_messages_ufn"] = { ufn:["activity_messages_ufn"] };

te["activity_messages_txt"] = { div:["ba_b"], c:
[
	{ div:[], c:
	[
		{ div:["d w13 g"], ac:["abs w12 x y ","","","xx y cd tr","Preset Replies"] },
		{ div:["e"] }
	]},
	{ div:[""], c:[ { textarea:["wp100 h09 ll r130 yy","","src_msg","","Reply ..."], ev:["","","_msg"] } ] }
]}

te["activity_message_r_v_o"] = { div:["ba"], c:
[ 
	{ s:["c x y b w15",null] },
	{ s:["c x y bl",null] }, 
	{ div:["e"] } 
]},

te["activity_message_r_v"] = { c:[ { pre:[":v:messages:src_vector::vector:11","",null] }, { div:["e"] } ] },

te["activity_message_r_"] = { c: 
[
	{ arg:["","","%0"] },
	{ div:[":v:messages:src_vector::vector:10"], c:
	[
		{ div:[], umime:["activity_message_r_v", ":v:messages:src_msg", ":v:messages:src_mime", ":v:messages:src"] },
		{ div:[":v:messages:src_vector::vector:12"], c:
		[
			{ div:[":v:messages:src_vector::vector:13"], s:["x bd gr cw m",":v:messages:src_status"] },
			{ s:[":v:messages:src_vector::vector:13", ":r::1:: : ago:: : ago:"] },
 			{ arg:["tm","","%1"] },
			{ div:[":v:messages:src_vector::vector:13"], s:[":v:messages:src_vector::vector:14",":v:messages:created_by"] },
			//{ arg:["tm","","%1"] },
			{ div:["e"] }
		]},
	]},
	{ div:["e"] }
]};

te["activity_message_r"] = { div:["y"], activity_message_r_:[] };

te["activity_messages_title"] = { c:
[
	{ s:["c ll t h3 b",":v:activities:src::case_src:1"] },
	{ s:["c x t h3","/"] },
	{ s:["c t h3 b",":v:activities:src_address"] },
	{ div:["d w10 t01 mr14"], s:["abs w10  gw zzzz",""], c:
	[
		{ div:["","ve"], c:
		[
			{ ac:["ay btn w09 ","","_ati_end","bd8 cb",""], c: 
			[
				{ s:["c x y cb s","End Chat"] },
				{ s:["c w03 h cb tc micon","last_page"] },
				{ div:["e"] }
			]}
		]}
	]}, 
	{ div:["e"] }
]};

te["activity_messages"] = { c:
[
	{ div:["x20 t20 b15"], u:["activity_messages_title","activities"] },
	{ div:[] }, // message reload ctx
	{ p:["x25","msgs"], c:
	[
		{ div:["xx yy oy scroller"], c:
		[
			{ u:["activity_message_r","messages","","","desc"] },
			{ ufn:["activity_messages_height"] },
		]},
		{ div:[] }
	]},
	{ div:["","ve"], c:
	[
		{ p:["x30 y20","o"], c:
		[
			 { div:[], c:
			 [
 				{ div:["d w08"], c:
 				[
 					{ ac:["abs btn ao t","","_activity_message_send"," t30 tc ba_b bd w08 h05 cb","Send"] },
 					{ div:["abs"], s:["w08 h05 t30 tc h05 go b savl","..."] } 
 				]},
 				{ div:["e"] }
			 ]},
			 { p:["mr10","sended"], activity_messages_txt:[] }
		]}
	]}
]};

// ------------------------------------------------------------------------------------------------

te["activity_case_ufn"] = { ufn:["activity_case_ufn"] };

te["activity_reporter_ufn"] = { ufn:["activity_reporter_ufn"] };

// ------------------------------------------------------------------------------------------------

te["activity_contact_disposition_btn"] = { div:["c"], c:
[
	{ ac:[null,"activity_disposition-contacts^disposition-@","_vp","x y03 cr bd",""], c:
	[
		{ s:["","# Disposition"] },
		{ div:["e"], arg:["",".id",null] }
	]},
	{ div:["g"], arg:["","","activity_disposition_r_dsp_new-dispositions-vf-vdisp-!"] }
]};

te["activity_contact"] = { div:["ba_b bd gw","va"], c: 
[ 
	{ ufn:["activity_contact_uuid_ufn"] },
	{ div:["","ve"], c:
	[
		{ div:["h08 oh ll"], c:
		[
			{ div:[], c:
			[
				{ div:["d w03"], c:
				[
					{ input:["g","",".id","-1","radio"] },
					{ ac:["abs w03 ab","activity_contact_none-r_","_rm","w02 x y03 tc h2 cb","&Cross;"] }
				]},
				{ div:["e"] }
			]},
			{ div:[], c:
			[
				{ ac:["au t","activity_contact_vw_id-contacts-@","_vp","cr",""], c:
				[
					{ s:["x y h3",":v:contacts:fullname"] },
					{ div:["e"], arg:["",".id","%0"] }
				]},
				{ div:[] }
			]},
			{ div:["x y03 s"], c:
			[
				{ uval:["",":v:contacts:location"] },
				{ div:["e"] }
			]},
		]},
		{ div:["x y"], c:
		[
			{ div:["c"], activity_contact_disposition_btn:["ab t","%0"] },
			{ ac:["d ab","case_new-reporters^uuid-vw","_activity_postj","xx y tr gr cw bd16",""], c:
			[
				{ s:["","New Case"] },
				{ p:["e","o"], arg:["","case_id","-1"] }
			]},
			{ div:["e"] }
		]},
		{ p:["e","nb"] }
	]}
]};

te["activity_contact_none"] = { div:["bd gws_"], c:
[
	{ ufn:["activity_contact_uuid_ufn"] },
	{ div:["x y h07"], c:
	[
		{ s:["c x y cd","Select Reporter"] },
		{ div:["e"] }
	]},
	{ div:["x y"], c:
	[
		{ div:["c"], activity_contact_disposition_btn:["ax","-1"] },
		{ div:["d"], c:
		[
			{ ac:["ax","case_contact_new-r_-^","_vp","x y03 bd cr",""], c:
			[
				{ s:["","+ New Contact"] },
				{ div:["e"], c:[ { arg:["","",""], ufn:["activity_src_address_ufn"] } ] }
			]},
			{ div:["g"], arg:["","","activity_contact_r_new-dispositions-vf-vcontactnew-!"] } 
		]},
		{ div:["e"] }
	]}
]};

// ---

te["activity_contact_footer"] = { div:["bt_ b05"], c:
[
	{ div:["d t"], c:[ { ac:["nav","activity_contact_ls-dispositions","_nav","dh bd",""], c:[ { div:["da_w dr bd"] }, { arg:["","_a","%0"] } ] }, { s:["navl","..."] } ] },
	{ div:["d t"], c:[ { aci:["nav","activity_contact_ls-dispositions","_nav","dh bd","prev",""], c:[ { div:["da_w dl bd"] }, { arg:["","_a","%0"] } ] }, { s:["navl","..."] } ] },
	{ s:["d x y cd s","%4"] },
	{ s:["d y cd s","of"] }, 
	{ s:["d x y cd s","%3"] },
	{ s:["d x y cd s","-"] },
	{ s:["d x y cd s","%2"] },
	{ div:["e"] }
]};
	
te["activity_contact_no_data"] = { c:
[
	{ s:["xx yy gy","No matching contacts found."] }
]};

te["activity_contact_r_"] = { div:["","va"], c:
[
	{ input:["g","",".id",":v:dispositions:reporter_contact_id","radio",null] },
	{ div:[null,"activity_contact-contacts-contacts-contact"], ev:["_u"], c:
	[
		{ p:["","nb"] }, 
		{ li:["x y oh cb r"], c:
		[
			{ div:["c l w01_ t08"], s:["opt",""] },
			{ div:["c w35"], contact_vw_rv:[":v:dispositions:reporter_fullname", "", "", "d x t cd s",":v:dispositions:created_on:d:dmyhnr", ":v:dispositions:reporter_location",""] },
			{ div:["e"] }
		]} 
	]}
]};

te["activity_contact_r_new"] = { div:[], c:
[
	{ activity_contact_r_:["1","r bt_ bb_ mtn1"] },
	{ div:["","nxfn"], arg:["","","activity_contact-contacts_disposition-contacts-contact"] } // cascading uvpfn
]},

te["activity_contact_r"] = { div:[], activity_contact_r_:["","r bt_"] };

te["activity_contact_k"] = { div:["g"], c:
[
	{ activity_disposition_k_:[] },
	{ arg:["","_c","%1"] },
	{ arg:["","sort","id"] },
	{ arg:["","group","reporter_contact_id"] },
	{ div:["e"] } 
]};
	 
te["activity_contact_nb"] = { div:[], c:
[
	{ u:["activity_contact_no_data","dispositions_no_data"] },
	{ p:["","vcontactnew"] }
]};

te["activity_contact_title"] = { div:[], c:
[
	{ s:["d xx y cd","Matching Reporters"] },
	{ div:["e"] }
]};

te["activity_contact_ls"] = { listo:["activity_contact_title", "activity_contact_nb", "", "activity_contact_k", "activity_contact_r", "dispositions", "activity_contact_footer"] };

// ---

te["activity_contacts"] = { div:["xx","contacts"], c:
[
	{ p:["","contact"], u:["activity_contact_none","activity_"] },
	{ div:["tt","vt"], u:["activity_contact_ls","dispositions_ctx"] }
]};

te["activity_contacts_none"] = { div:["xx","contacts"], c:
[
	{ p:["","contact"], u:["activity_contact_none","activity_"] },
	{ div:["t20","vt"], c:[ { p:["","vcontactnew"] } ] }
]};

te["activity_list_contacts"] = { c:
[
	{ activity_f_tags_k:[] },
     { arg:["","_c","5"] },
	{ arg:["","sort","id"] },
	{ arg:["","group","reporter_contact_id"] },
	{ uv:["activity_contacts","dispositions",":k:activities_k:id:2"] }
]};

// -------------------------------------------------------------

te["activity_disposition_category_r"] = { div:["",""], c:
[
	{ input:["g","","disposition_id","%0","radio"] },
	{ div:["r ay bd_"], ev:["_opt"], c:[ { li:["xx t02 cb"], c:
	[
		{ div:["c w01_ t07"], s:["opt",""] },
		{ div:["c x y w40"], uval:["",":v:categories:fullname"] },
		{ div:["e"] }			
	]} ]}
]};

te["activity_disposition_contact_known"] = { c:
[
	{ div:["b20"], c:
	[
		{ contact_vw_rv:[":v:contacts:fullname", "", "", "","", ":v:contacts:location",""] },
		{ p:["","o"], arg:["","contact_id","%0"] }
	]}
]};

te["activity_disposition_contact_unknown"] = { c:
[
	
]};

te["activity_disposition_contact"] = { u:[":u::0:0:activity_disposition_contact_unknown:activity_disposition_contact_known"] };

te["activity_disposition_contact_btn"] = { ac:["ao btn",":u::0:0:activity_disposition_unk-dispositions^unk:activity_disposition-dispositions","_activity_postj","y07 gb bd cw b n tc","Disposition"] };

te["activity_disposition"] = { div:["w50 x y ma sh__ gw_ bd8"], ev:["_undd"], c:
[
	{ div:["xx"], c:
	[
		{ s:["c xx y15 b h3","Disposition"] },
		{ ac:["d","","_uvp","xx y08 h cb","&Cross;"] },
		{ div:["e"] }
	]},

	{ div:["","ve"], s:["x15 t",""], c:
	[
		{ div:["gp"], c:[ { p:["","nb"] }, { div:["e"] } ] },

		{ u:["activity_disposition_contact","contacts_disposition"] },	

		{ p:["h20 oy ba_ bd mb2","o"], u:["activity_disposition_category_r","subcategories"] },

		{ div:["yy"], c:
		[
			{ u:["activity_disposition_contact_btn","contacts_disposition"] },	
			{ s:["y07 bd b tc gws_ bd savl","..."] }
		]}
	]}
]};

// ---

te["activity_disposition_vw_case"] = { c:
[
	{ div:["tt"], c:
	[
		{ s:["c x y b",CASE_ID_PREFIX] },
		{ s:["c y b",":v:cases:id"] },
		{ p:["e","o"], arg:["","case_id","%0"] }
	]},
	{ div:["tt"], c:
	[
		{ s:["x t cd","Case Category"] },
		{ div:["x t"], uval:["",":v:cases:case_category"] },
		{ div:["e"] }
	]},
	{ div:["tt"], c:
	[
		{ s:["x t cd","Case Narrative"] },
		{ div:["x t"], s:["",":v:cases:narrative"] },
		{ div:["e"] }
	]},
	{ div:["tt"], c:
	[
		{ s:["x t cd","Case Plan"] },
		{ div:["x t"], s:["",":v:cases:plan"] },
		{ div:["e"] }
	]},
	{ div:["yy r20"], c:
	[
		{ div:["c"], c:[ { s:["x t cd","Priority"] }, { s:["x t",":v:cases:priority::case_priority:2"] } ] },
		{ div:["d"], c:[ { s:["x t cd tr","Status"] }, { s:["x t tr",":v:cases:status::case_status:1"] } ] },
		{ div:["e"] }
	]},
	
	// { div:[], usub:["case_vw_refered_to_none,case_vw_refered_to_dept_sub,case_vw_refered_to_sub","cases",":v:cases:is_refered_to","0,1,2"] },

	// { div:[], usub:["case_vw_escalated_to_sub","cases",":v:cases:is_escalated_to","1"] } 
]};

te["activity_disposition_vw"] = { div:["w60 x15 yy ma sh__ gw_ bd8","vddvw"], ev:["_undd"], c:
[
	{ u:["activity_disposition_vw_case","cases"] } 
]};

// ------

te["activity_disposition_footer"] = { div:["x25 y20"], s:["l01",""], c:
[
	{ pg:["pgto","activity_disposition_list-dispositions"," dh","da dl","activity_disposition_list-dispositions"," dh","da dr"] },
	{ div:["e"] }
]};

te["activity_disposition_r_case_details"] = { c:
[
	{ s:["c micon cd","navigate_next"] },
	{ s:["c l b",CASE_ID_PREFIX] },
	{ s:["c r05 b",":v:dispositions:case_id"] },
	{ s:["c micon cd","navigate_next"] },
	{ div:["c x "], uval:["",":v:dispositions:case_category"] },
	{ div:["g"], arg:["","case_id",":v:dispositions:case_id"] }
]};

te["activity_disposition_r_"] = { c:
[
	{ div:["abs l03 t"], s:["x y micon h3 ba gws_ bd16",":v:dispositions:src::case_src:6"] },
	{ div:["ml1_ mh05 l20 r10 t08 bl r"], c:
	[
		{ div:[], c:
		[
			{ s:["c l cd s",":v:dispositions:created_by"] },
			{ s:["c l cd s",":r::1:: : ago:: : ago:"] },
			{ arg:["tm","","%1"] },
			{ div:["e"] }
		]},
		{ div:["y02"], c:
		[
			{ div:["c x "], uval:["",":v:dispositions:disposition"] },	
			{ u:[null] },
			{ u:[":v:dispositions:reporter_contact_id:u:::0:noop:activity_disposition_r_fullname"] },	
			{ div:["e"] }	
		]}
	]},
	{ p:["","nb"] } // nb
]};

te["activity_disposition_r_fullname"] = { div:["c l r10"], s:["x y03 gws_ bd8 mtn3",":v:dispositions:reporter_fullname"] };

te["activity_disposition_r_case_"] = { div:["l20","va"], c:
[
     { input:["g","","advwt","%0","radio"] },
	{ li:[null,"activity_disposition_vw-cases"], ev:["_activity_disposition_r"], activity_disposition_r_:["activity_disposition_r_case_details"] },
     { div:["g"], arg:["","","activity_disposition_r-dispositions-va--@"] }, // vp return anchor
]};

te["activity_disposition_r_dsp_"] 		= { div:[null,"va"], activity_disposition_r_:["noop"] };

te["activity_disposition_r_case_new"] 	= { activity_disposition_r_case_:["lr gh bd"] };

te["activity_disposition_r_case"] 		= { activity_disposition_r_case_:["lr bd"] };

te["activity_disposition_r_dsp_new"] 	= { activity_disposition_r_dsp_:["ml2 lr gy"] };

te["activity_disposition_r_dsp"] 		= { activity_disposition_r_dsp_:["ml2 lr"] };

te["activity_disposition_r"] 			= { u:[":v:dispositions:case_id:u:::0:activity_disposition_r_dsp:activity_disposition_r_case"] };

te["activity_disposition_k"] = { div:["g"], c:
[
	{ activity_disposition_k_:[] },
	{ arg:["","sort","id"] },
	{ arg:["","_c","%1"] }, 
	{ div:["e"] }
]};

te["activity_disposition_no_data"] = { div:[""], s:["x30 yy cd","No Records"] };

te["activity_disposition_nb"] = { div:[""], u:["activity_disposition_no_data","dispositions_no_data"] };

te["activity_disposition_title"] = { div:[], c:[ { p:["","vdisp"] } ] };

te["activity_disposition_list"] = { list:["activity_disposition_title", "activity_disposition_nb", "", "activity_disposition_k", "activity_disposition_r", "dispositions", "activity_disposition_footer"] }; // :u::4:0:noop:

te["activity_list"] = { c:
[
	{ div:["d w40 r15 t01"], s:["abs w40 tt b20",""], u:[":u::4:0:activity_contacts_none:activity_list_contacts"] },
     { div:["mh30 mr43 tt","vt"], activity_disposition_list:[] },
	{ div:["e"] }
]};

te["activity_main"] = { c: 
[
	{ div:["ll r30 yy","vb"], c:
	[
		{ div:["c"], c:
		[
			{ div:["tt","va"], s:["",""], c:
			[
				{ input:["g","","cases_t_","0","radio","1"] },
				{ ac:["c x","activity_match-activities-vftab","___u","xx y b h3 cb","Contact History"] }, 	
				{ div:["e"] }
			]}
		]},

		{ div:["c t12"], c:
		[
			{ input:["g","","activity_vw_id_t_","0","radio"] },
			{ ac:["ay tabb ll","activity_f-dispositions_f","_vpf","x y02 bd8 cb",""], c: // contextual search: dispositions | messages
			[
				{ s:["c h2 b micon","search"] },
				{ div:["c x t01 s","","Search"] }, 
				{ div:["e"] }
			]}
		]},
		
		{ div:["e"], c:[ { arg:["","","activity_list-dispositions"] }, { arg:["","","0"] }, { arg:["","","0"] }, { arg:["","",""] } ] }
	]},

	{ div:["x25 y" ,"vf"], c:[ { div:["","activity_disposition_f-dispositions_f"], ev:["l__vpf"], c:
	[
		{ activity_f_tags_k:[] }
	]} ]},
	
	{ div:["t"], c: 
	[
		{ div:[], c:
		[ 
			{ input:["g","","cghgmtv","0","radio","1"] }, 
			{ p:["tabv","vt"], u:["activity_list"] } 
		]},
		{ div:[], c:
		[	
			{ input:["g","","cghgmtv","0","radio"] }, 	// activity history rpt
			{ p:["tabv","vt"] } 
		]}
	]}
]};

// ------

te["activity_vw_id_toolbar"] = { c:
[
	{ div:[], c:
	[
		{ div:["d w08 t01"], s:["abs w08 bd8 t20 b10 gw zzzzz",""], c:
		[
			{ input:["g","","sbl","0","radio"] },
			{ ac:["ay t01 r15 w03 ma","","_activity_close","cb bd y01",""], c:
			[
				{ s:["tc h b","&Cross;"] },
				// { s:["d x y s","Close"] },
				{ div:["e"] }
			]}
		]},
		{ div:["e"] }
	]}
]};

te["activity_vw_id_toolbar_walkin"] = { c:
[
	{ div:["w16 ma t01"], s:["w12 t20 b10 abs",""], c:
	[
		{ div:["xx y w10 bd cd s g"], c:
		[
			{ s:["c","Walkin"] },
			{ s:["c x","0:00"] }, // todo: add timer
			{ arg:["ts","","0"] }, 
			{ div:["e"] }
		]},
	]},

	{ div:[], c:
	[
		{ div:["d w08 t01"], s:["abs w08 bd8 y20 gw zzzzz",""], c:
		[
			{ input:["g","","sbl","0","radio"] },
			{ ac:["ay t01 r15 w03 ma","","_activity_close","cb bd y01",""], c:
			[
				{ s:["tc h b","&Cross;"] },
				// { s:["d x y s","Close"] },
				{ div:["e"] }
			]}
		]},
		{ div:["e"] }
	]}
]};

te["activity_vw_id_args"] = { c:
[
    { arg:["","src",":v:activities:src"] },
    { arg:["","src_uid",":v:activities:src_uid"] },
    { arg:["","src_address",":v:activities:src_address"] },
    { arg:["","src_uid2",":v:activities:src_uid2"] },
    { arg:["","src_usr",":v:activities:src_usr"] },
    { arg:["","src_vector",":v:activities:src_vector"] },
    { arg:["","src_callid",":v:activities:src_callid"] },
    { arg:["","src_ts",":v:activities:src_ts"] },
]};

te["activity_vw_id_tabs_0_walkin"] = { u:["activity_main","ctx_"] };

te["activity_vw_id_tabs_0"] = { c:
[
	{ arg:["",":v:activities:src::case_src:12",":v:activities:src_address"] },
	{ arg:["","_c","10"] },
	{ uv:["activity_main","dispositions"] }
]};

te["activity_vw_id_"] = { c: 
[
	{ div:["","vb"], c:
	[
		{ u:[null] }, 											// channel menu
		{ div:[], c:[ { p:["g","o"], c:
		[
			{ activity_vw_id_args:[] }, 
			{ arg:["","activity_id",":v:activities:id"] },
			{ arg:["","contact_uuid_id","-1"] },
			{ arg:["","activity_ca_id",":v:activities:ca_id"] },
		]} ]}
	]},
	{ form:[], c:
	[
		{ div:[], c:
		[	
			{ input:["g","","activity_vw_vt","0","radio",null] }, 		// activity history list
			{ p:["tabv","vftab"], u:[null] } 
		]},
		{ div:[], c:
		[	
			{ input:["g","","activity_vw_vt","0","radio",null] }, 		// case_form | case_vw_id
			{ p:["tabv",null], u:["case_vw_id","cases"] } 
		]},
		{ div:[], c:
		[	
			{ input:["g","","activity_vw_vt","0","radio",null] }, 		// chat
			{ p:["tabv","vf"], u:[null] } // activity_messages 
		]}	
	]}
]};

te["activity_vw_id_walkin"] 	= { activity_vw_id_:["activity_vw_id_toolbar_walkin","1","activity_vw_id_tabs_0_walkin","","vf","","activity_messages"] }; 
te["activity_vw_id_case"] 	= { activity_vw_id_:["activity_vw_id_toolbar","","noop","1","vfvw","","activity_messages"] }; 
te["activity_vw_id_notif"] 	= { activity_vw_id_:["activity_vw_id_toolbar","","noop","1","vfvn","","activity_messages"] }; 
te["activity_vw_id_msg"] 	= { activity_vw_id_:["ati_toolbar","","activity_vw_id_tabs_0","","vf","1","pmessage_vw_id"] }; 
te["activity_vw_id_chat"] 	= { activity_vw_id_:["ati_toolbar","","activity_vw_id_tabs_0","","vf","1","activity_messages"] }; 
te["activity_vw_id_call"] 	= { activity_vw_id_:["call_toolbar","1","activity_vw_id_tabs_0","","vf","","activity_messages"] }; 

// ------

te["activity_lst_footer"] = { div:["xx y bt_"], c:
[
	{ div:["d t"], c:[ { ac:["nav","activity_lst-activities","_nav","dh",""], c:[ { div:["da dr_"] }, { arg:["","_a","%0"] } ] }, { s:["navl","..."] } ] },
	{ div:["d t l"], c:[ { aci:["nav","activity_lst-activities","_nav","dh","prev",""], c:[ { div:["da dl_"] }, { arg:["","_a","%0"] } ] }, { s:["navl","..."] } ] },
	{ s:["d x y cd s","%4"] },
	{ s:["d  y cd s","of"] }, 
	{ s:["d x y cd s","%3"] },
	{ s:["d x y cd s","-"] },
	{ s:["d x y cd s","%2"] },
	{ div:["e"] }
]};

te["activity_lst_r_disposition"] = { div:["c r05 t02 s cr"], uval:["","%0"] };

te["activity_lst_r_case_id"] = { c:
[
	{ s:["c t","#"] },
	{ s:["c t",":v:activities:case_id"] },
	{ s:["c l t",":v:activities:src::case_src:2"] },
	{ s:["c l t",":v:activities:src_address"] },
	{ div:["d mt"], s:[":v:activities:action::activity_action:3",":v:activities:action::activity_action:2"] },
]};

te["activity_lst_r"] = { p:["",":v:activities:src_uid"], c:
[
	{ input:["g","","sbr",":v:activities:src_uid","radio",":k:activities_chk:src_uid::14"] },
	{ li:["sbr xx t03 bt_ s oh h05"], ev:["_activity_vw_id"], c: 
	[
		{ div:[], c:
		[  
			{ s:["c t",":v:activities:src::case_src:1"] },
			{ s:["c l t",":v:activities:src_vector::vector:6"] },
			{ s:["c l t",":v:activities:src_address"] },
			{ s:["d t",":v:activities:src_status::activity_status:1"] },
			{ s:["d t",""] }, // count
			{ usub:
			[
				"activity_lst_r_case_id,activity_lst_r_case_id",
				"r_",
				":v:activities:src",
				"update,escalation"
			]},
			{ div:["e"] }
		]},
		{ div:["h02_ "], c:
		[	
			{ s:["d t",":v:activities:src_ts:r:::: : ago:: : ago:"] },
			{ arg:["tm","",":v:activities:src_ts"] },
			{ uchk:["activity_lst_r_disposition",":v:activities:dispositions"] }, 
			{ div:["e"] }
		]},
		{ div:["g"], c:
		[ 
			{ arg:["",".id","%0"] }, 
			{ arg:["","src",":v:activities:src"] }, 
			{ arg:["","action",":v:activities:action"] }, 
			{ div:[], usub:
			[
				"activity_agtk_call,activity_agtk_chat,activity_agtk_chat",
				"r_",
				":v:activities:src",
				"call,chat,whatsApp"
			]}
		]},
	]}
]};

te["activity_lst_k"] = { c:
[
	// todo: filter args here
	{ arg:["","_aa","%0"] } // remember current start offset of current page
]};

te["activity_lst_title"] = { div:[""], c:
[
	{ s:["c xx yy b","Notifications"] }, // Activity History
	{ div:["e"] }
]}; 

te["activity_lst"] = { list:["activity_lst_title","end","","activity_lst_k","activity_lst_r","activities","activity_lst_footer"] }; // sbr panel

// ------

te["activity_vw_id_ufn"] = 	{ ufn:["activity_vw_id_ufn"] };

te["activity_lst_ufn"] = 	{ ufn:["activity_lst_ufn"] };

te["activity_agtk_chat"] =  	{ ufn:["activity_agtk_chat_ufn"] };

te["activity_agtk_call"] =  	{ ufn:["activity_agtk_call_ufn"] };

// ------

function activity_messages_height (el, u, a, r, m)
{
	el.style.height = window.innerHeight-210;
	var h = el.scrollHeight;
	// console.log (" [scroll height ] "+h);
	el.scroll ({ top: h, left: 0, behavior: 'smooth' }); 
}

function activity_messages_ufn (el, u, a, r, m)
{
	var p = el.nextSibling.firstChild; 
	var el = p.lastChild;
	var id = 0;
	if (p.lastChild && p.lastChild.firstChild) id = p.lastChild.firstChild.value*1;
	var rr = ra["messages"];
	var n = rr.length;
	for (var i=n-1; i>-1; i--)
	{
		var r = rr[i];
		if ((r[0]*1)<=id) continue;
		nd (p, te["activity_message_r"], [], r, [0]); // append new messages 
	}
	//console.log (" ---> "+el);
	//for (var i=0; i<n; i++)	// update read status
	//{
	//	var r = rr[i];
	//	//console.log (" ---> "+r[0]+","+el.firstChild.value + "|"+i+" of "+n)
	//	if (!el) break;
	//	if ((r[0]*1) > (el.firstChild.value*1)) continue;
	//	if ((r[0]*1) == (el.firstChild.value*1)) 
	//	{
	//		_(el,"tm","input").parentNode.firstChild.firstChild.innerHTML = r[15];
	//		el = el.previousSibling
	//	}
	//}
	activity_messages_height (p, u, a, r, m);
}

function activity_message_sended (el, u, a, r, m)
{
	var p = __(el,"ve");
	var k = ra["messages_k"]
	var el = nd (el, te["activity_messages_txt"], [], [], [0]);
	el.focus (); // console.error (el.tagName)
	url (p.parentNode.childNodes[1], "activity_messages", "messages", ("?src="+r[k["src"][0]]+"&src_callid="+r[k["src_callid"][0]]+"&"));
}

function _activity_message_send (ev)
{
	var p = __(this,"ve");
	var p_ = __(this,"vfvwm");
	var o = {};
	jso (p_.firstChild, o);   	// channel session details
	jso (p,o); 			// ve details
	url (p, "activity_message_send", "messages", "", null, 2, o, "POST");
	boo (ev)
}

function _msg (ev) // TODO: send read request -while typing - to keep chat from timimg out
{
	// console.error (ev.shiftKey+", "+ev.keyCode)
	if (ev.shiftKey==false && ev.keyCode==13)
	{
		this.disabled = true;
		var p = __(this,"ve");
		var p_ = __(this,"vfvwm");
		var o = {};
		jso (p_.firstChild.lastChild, o);   	// channel session details
		jso (p,o); 						// ve details
		url (p, "activity_message_send", "messages", "", null, 2, o, "POST");
		return false;
	}
}

// -------------------------------------------------------------

function activity_lst ()
{
	var pu = document.getElementById ("vt_activity");
	var a = {};
	// console.log ("pu:"+pu.childNodes.length+"------------------------------------")
	argv (pu.childNodes[2].childNodes[1], a);						// retrieve clicked/loaded record
	re["activities_chk"] = { "src_uid": {} };
	if (a.sbr) re["activities_chk"]["src_uid"][a.sbr] = 1;
	a = {args:"?", "_aa":null};
	argv (pu.childNodes[2].firstChild, a);
	url (pu, "activity_lst", "activities", a.args+"&_a="+a["_aa_"].value, null, 1); // maintain context
}

function activity_lst_ufn (el, u, a, r, m)
{
	activity_lst ();
}

function activity_vw_id_ufn  (el, u, a, r, m)
{
	let src = r[re["activities_k"]["src"][0]];
	let p = document.getElementById ("vv").childNodes[6].childNodes[1].childNodes[1].childNodes[1].childNodes[1];
	p.innerHTML = "";
	nd (p, te[ ("activity_vw_id"+re["case_src"][src][10])], [], r, [0])
}

function activity_agtk_chat_ufn (el, u, a, r, m)
{
	//console.log (">>agtk>chat")
	let src_uid = r[re["activities_k"]["src_uid"][0]];
	let ch = re["atis"][src_uid]
	let pcoll = document.getElementById ("vv").childNodes
	// console.log ("agtk("+src+") "+id+" "+(ch?"ch"+ch[AMI.CHAN_STATE_HANGUP]:"null")+"--------------------")
	if (ch && ch[ATI.CHAN_STATE_HANGUP].length==0)
	{
		chan_status ("ati_agtk", ch);
		ati_agtk (el.parentNode.parentNode.parentNode, ch, (pcoll.length>6 ? pcoll[6].childNodes[1].childNodes[1] : null))
	}
}

function activity_agtk_call_ufn (el, u, a, r, m)
{
	//console.log (">>agtk>call")
	let src_uid = r[re["activities_k"]["src_uid"][0]];
	let ch = re["channels"][src_uid]
	let pcoll = document.getElementById ("vv").childNodes
	// console.log ("agtk("+src+") "+id+" "+(ch?"ch"+ch[AMI.CHAN_STATE_HANGUP]:"null")+"--------------------")
	if (ch && ch[AMI.CHAN_STATE_HANGUP].length==0)
	{
		chan_status ("chan_args", ch);
		chan_agtk (el.parentNode.parentNode.parentNode, ch, (pcoll.length>6 ? pcoll[6].childNodes[1].childNodes[1] : null))
	}
}

function activity_case_ufn (el, u, a, r, m)
{
	var p = __(el,"vf");
	p.innerHTML = "";
	nd (p, te["case_vw_id"], [], ra["cases"][0], [0]);
	if (p.id.length==4)					// case_notification, case_vw_id
	{
		if (p.id=="vfvn") return; 		// dont switch tabs for case_notifications
		var a = {};
		var b = {};
		var coll = __(p,"vfvwm").parentNode.parentNode.firstChild.childNodes
		coll[0].checked = true; 			// switch tab
		argv (coll[1].lastChild.firstChild.childNodes[1] .childNodes[2].childNodes[1], a, "name", null, b)
		if (b.casevwr && b.casevwr.length>0)
		{
			p = b.casevwr[0].parentNode;
			p.innerHTML = ""
			nd (p, te["case_r_r"], [], ra["cases"][0], [0])
		}
		return
	}
	var el_ = document.createElementNS ("http://www.w3.org/1999/xhtml", "div");
	var coll = p.parentNode.parentNode.firstChild.childNodes
	coll[0].checked = true; 	
	nd (el_, te["activity_disposition_r_case_new"], [], r, [0]);
	p = _(coll[1],"vdisp"); 				// disposition container
	p.insertBefore (el_, p.firstChild);
}

function activity_contact_uuid_ufn (el, u, a, r, m)
{
	var o = { contact_uuid_id:null };
	argv (__(el,"vfvwm").firstChild.lastChild, o);						// src
	o.contact_uuid_id_.value = r[0];
}

function activity_src_address_ufn (el, u, a, r, m)
{
	var o = {};
	argv (__(el,"vfvwm").firstChild.lastChild, o);						// src
	el.id = "contacts-"+re["contacts_k"][re["case_src"][o.src][11]][0]
	el.value = o.src_address
}

// ---

function activity_close (p)
{
	p.innerHTML = ""
	p.parentNode.previousSibling.firstChild.checked = true;
	document.getElementById ("vv").childNodes[2].lastChild.firstChild.checked = true; // uncheck sbr
}

function _activity_close (ev)
{
	var p = __(this,"vf");
	if (p.id.length==2)
	{
		p.parentNode.parentNode.firstChild.firstChild.checked = true;
		return;
	}
	if (p.id!="vfvwm") p = __(p,"vf");
	activity_close (p);
	boo(ev)
}

function _activity_postj ()
{
	var u = this.id.split ("-");
	var p = __(this,"ve"); 
	var o = {};
	var ld = 2
	jso (__(elvpf?elvpf:(elvp?elvp:p),"vfvwm").firstChild.lastChild, o);		// src
	jso (p, o); 													// form
	if (u.length>2 && u[2]=="vp")
	{
		p = document.getElementById ("vp");
		elvp = this.nextSibling
		vp (p)
		p.innerHTML = ""; // "<div id='ve'></div>"
		// p = p.firstChild
		ld = 0
	}
	if (u.length>2 && u[2]=="vw")
	{
		p = __(this,"vf").parentNode.nextSibling.childNodes[1]
		p.previousSibling.checked = true
		ld = 0
	}
	url (p, u[0], u[1], o[".id"], null, ld, o, "POST");
}

function _activity_disposition_r ()
{
	var u = this.id.split ("-");
	var o = {};
	this.previousSibling.checked = true;
	jso (__(this,"vfvwm").firstChild.lastChild, o);		// src
	argv (this, o);								// data
	if (o.contact_uuid_id && o.contact_uuid_id>0)
	{
		var coll = __(this,"vf").parentNode.nextSibling.childNodes;
		coll[0].checked = true
		coll[1].innerHTML = ""
		url (coll[1], "case_vw_id", u[1], o.case_id); 
		return
	}
	var p = document.getElementById ("vp");
	elvp = this.nextSibling;
	vp (p)
	url (p, u[0], u[1], o.case_id); 					// todo: load audit trail instead of case
}

function _activity_vw_id (ev) 
{
	var pcoll = document.getElementById ("vv").childNodes
	var coll = pcoll[6].childNodes[1].childNodes[1].childNodes[1].childNodes;
	var a = {".id":0};
	var s = "";
	var u = ["activity_vw_id","activities"]
	argv (this, a); 
	u[0] = "activity_vw_id"+re["case_src"][a.src][10] 
	
	if ((a[".id"]*1)<1) 
	{ 
		var user_cid = document.getElementById ("user_cid").value;
		a.src_ts = Date.now()/1000;
		a.src_uid = a.src+"-"+user_cid+"-"+Date.now ();
		if (a.src_uid2==undefined) a.src_uid2 = a.src_uid+"-1";
		if (a.src_callid==undefined) a.src_callid = a.src_uid2;
		a.src_usr = user_cid
		a.src_vector = 1;
		a[".id"] = -1;
		s ="?src=" + a.src + "&src_uid=" + a.src_uid + "&src_uid2=" + a.src_uid2 + "&src_callid="+a.src_callid + "&src_vector=" + a.src_vector;
		s += "&src_usr="+a.src_usr;
		if (a.src_address && a.src_address>0) s += "&src_address="+a.src_address;
		if (a.case_id && a.case_id.length>0) s += "&case_id="+a.case_id;
	}

	if (this.id.length>0)
	{ 
		u[0] = "activity_vw_id_"+this.id
		coll = __(this,"vftab").parentNode.nextSibling.childNodes;
	}
	else
	{
		pcoll[3].childNodes[1].firstChild.checked = true;
		pcoll[6].childNodes[1].firstChild.checked = true;
	}
	this.previousSibling.checked = true; // hilite call-notif
	coll[0].checked = true;

	if (a.src && a.action && (a.src=="update" || a.src=="escalation") && a.action=="notify") // read
	{
		// todo: set to loading state
		var pn = document.getElementById ("notif_count").firstChild;
		var o = { "action":"complete" }
		url (pn, "activity_upd", "activities", a[".id"], null, 2, o, "POST");
		boo (ev);
		return;
	}

	url (coll[1], u[0], u[1], a[".id"]+s);
	boo(ev)
}

function _activity_call () // outbound call popup
{
	var p = document.getElementById ("vp");
	var a = {};
	argv (__(this), a);
	p.style.display = "none";
	p.innerHTML = "";
	VOICEAPPS_UA.dial(a.src_address)
}