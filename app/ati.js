...
function atis (o,k,ts)
{
	...
	for (var i=k.length-1; i>-1; i--)
	{
		...
		{
			var a_ = {};
			if (pv.firstChild && pv.firstChild.lastChild) argv(pv.firstChild.lastChild, a_);

			// console.log ("aiipanel --> "+ch[ATI.CHAN_BRIDGE_ID]+" |" + JSON.stringify (a_));

			if (a_.src_uid == ch[ATI.CHAN_BRIDGE_ID] || a_.src_uid2 == ch[ATI.CHAN_BRIDGE_ID])
			{
				console.log ("---   aii_panel ----------")
			...
			//	// todo: highlight icon
				...
				url (coll_[2].childNodes[1], "case_insights","messages",  ("?src_callid="+ch[ATI.CHAN_BRIDGE_ID]+"&_c=10&sort=id")); // pick latest
